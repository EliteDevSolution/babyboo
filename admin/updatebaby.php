<?php 
  ...
  {
    $no = $_GET['no'];
    $curRes = viewOneData("Select * from bayi where No='$no'", $conn);
    if (isset($_POST['submit'])) {
      $tarikh =  $_POST['Tarikh'];
      $lokasi = $_POST['Lokasi'];
      $statusbaby = $_POST['StatusBaby'];
      $organ = $_POST['Orang'];
      $nofon = $_POST['NoFon'];
      $sql = "Update bayi set Tarikh='$tarikh', Lokasi='$lokasi', Status='$statusbaby', Orang='$organ', NoFon='$nofon' where No='$no'";
      $stmt= $conn->prepare($sql);
      $stmt->execute();
      echo "<script>alert('Updated successfully.');document.location.href='listbaby.php'</script>";
    }
  }
?>
    ...
    <div class="br-mainpanel">
      ...
      <div class="br-pagebody">
        <!-- start you own content here -->
              <form action="updatebaby.php?no=<?=$no?>" method="post" data-parsley-validate>
              <div class="form-layout form-layout-4" >
                <h6 class="tx-gray-800 tx-uppercase tx-bold tx-14 mg-b-10" align="center">Update Record</h6>
                <br>
                <div class="row">
                  <label class="col-sm-4 form-control-label">Date: <span class="tx-danger">*</span></label>
                  <div class="col-sm-8 mg-t-10 mg-sm-t-0">
                    <input type="date" name="Tarikh" class="form-control wd-250" placeholder="Enter date" value="<?=$curRes['Tarikh']?>" required>
                  </div>
                </div>
                <div class="row mg-t-20">
                  <label class="col-sm-4 form-control-label">Location Found: </span></label>
                  <div class="col-sm-8 mg-t-10 mg-sm-t-0">
                    <input type="text" name="Lokasi" class="form-control wd-600" placeholder="Enter location baby was found" value="<?=$curRes['Lokasi']?>">
                  </div>
                </div>
                <div class="row mg-t-20">
                  <label class="col-sm-4 form-control-label">Status Of Baby: </label>
                  <div class="col-sm-8 mg-t-10 mg-sm-t-0">
                    <input type="text" name="StatusBaby" class="form-control wd-300" placeholder="Status of baby at the moment " value="<?=$curRes['Status']?>"></input>
                  </div>
                </div>
                <div class="row mg-t-20">
                  <label class="col-sm-4 form-control-label">Baby Found By: <span class="tx-danger">*</span></label>
                  <div class="col-sm-8 mg-t-10 mg-sm-t-0">
                    <input type="text" name="Orang" class="form-control wd-250" placeholder="Enter your name" required value="<?=$curRes['Orang']?>">
                  </div>
                </div>
                <div class="row mg-t-20">
                  <label class="col-sm-4 form-control-label">Phone-No: </label>
                  <div class="col-sm-8 mg-t-10 mg-sm-t-0">
                    <input type="tel" name="NoFon" class="form-control wd-300" placeholder="Enter your phone number" value="<?=$curRes['NoFon']?>"></input>
                  </div>
                </div>
